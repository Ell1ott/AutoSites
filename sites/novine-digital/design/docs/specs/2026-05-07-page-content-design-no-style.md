...
Structure and presentation are out of scope here.
...
Four demo sites exist (none sold to real clients yet). They're shown in the Gallerie with **plausible made-up names** to imply pursuit/portfolio without lying about contracts.
...
**Message:** One bundled offer — everything handled — rather than four separate services.
...
**Status:** Existing section in `design.html` (approx. lines 626–684). **No structural change to the section shell.** Labels and sub-line are new.
...
Cards are **decorative only** — not links to project pages.
...
**Job:** Remove the "how much of MY time will this eat?" fear that busy café owners bring to cold outreach. Their involvement should read as minimal.
...
**Copy constraint:** **No week labels** (timeline isn't fixed enough to commit to dates).

### Notes

- *"Ingenting"* in step 04's "Du gør" column is the punchline.
...
The agency speaking *to* the cold-reading café owner. No attribution on the quote.
...
- **Direct email:** `elliott@novine.dk` (inside the founder block, in addition to the dedicated CTA section below — "direct line to the founder" is a deliberate trust signal for cold outreach)
...
(TBD — something neutral like "Spørgsmål, vi ofte får" or simply "FAQ".)
...
**Job:** Frictionless final step. Clear primary contact moment before the footer.
...
These threads appear deliberately in multiple sections. Preserve them as edits are made:

1. **"Everything handled."** Section 2 headline ("Vi tager os af det hele"), Section 4 (minimal "Du gør" burden), Section 7 ("ingen forpligtelse").
...
- **Layout, styling, and responsive behavior.** Handled at implementation time.
