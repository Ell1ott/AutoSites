-- Public bucket for CMS-uploaded images.
...
-- Anyone can read (bucket is already public, but policy makes it explicit).
...
create policy "cms-images admin insert"
  ...
  with check (
    ...
    and exists (select 1 from public.cms_admins where user_id = auth.uid())
  );
